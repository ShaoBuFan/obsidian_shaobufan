...
闭包不是某个工程师拍脑袋设计的。它来自数学——具体说，来自阿隆佐·丘奇（Alonzo Church）在 1936 年提出的 **λ 演算**（Lambda Calculus）。要理解闭包的根源，需要理解 λ 演算试图解决什么问题。
...
| 1975 | Guy Steele 和 Gerald Sussman 设计 **Scheme**（LISP 方言），采用词法作用域 |
...
闭包之所以能访问外部变量，是因为 JavaScript 的作用域链机制：
...
| 需要读值 | `setIsOpen(prev => !prev)` 用函数式更新 |
