...
>
> **重要**：如果你之前习惯用类组件（`componentDidMount` / `componentDidUpdate` / `componentWillUnmount`）思考 React，本模块要求你先搁置那些概念。Hook 不是生命周期的替代品——它是基于闭包和调用顺序的完全不同的心智模型。用旧框架理解新东西，是大多数困惑的来源。
...
2. 什么是==闭包==？你能在 30 秒内向一个初级开发者解释清楚吗？
...
**第二层：==Hook 链表==层面（用 Module 0 后面的知识）**
