...
==六步设计法==的目的就是给你一个**构建流程**——用科学的步骤代替灵感，让你每次"从零设计"时知道第一步做什么、第二步做什么、每一步的产出是什么。
...
- **卸载**：`useEffect` 的==清理函数==移除事件监听

**检查清单：**

| 检查项 | 状态 |
|--------|------|
| 有需要清理的==副作用==吗？ | ✅ 事件监听已清理 |
| 有遗漏的依赖吗？ | ✅ ==deps 数组== `[]` 是正确的（事件监听器不变） |
| 有==过期闭包==吗？ | ✅ handleOnline/handleOffline 内部调用的 `setIsOnline` 是 dispatch 函数，不会过期 |
...
**Q2：模式是 Step 4 的分类标签，不是捷径。**

六步法和五大模式的关系可以用一句话概括：**六步法是设计流程，五大模式是设计结果**。
- 六步法告诉你"怎么一步步设计"，输入是什么、每一步产出什么。
- 五大模式告诉你"你设计出来的东西属于哪一类"，方便你沟通和复用。

更精确地说，模式是 Step 4（选择原语）产物的分类标签。你在 Step 1-3 中分析了需求（识别值、分类、定契约），到 Step 4 时才决定"用什么 Hook、怎么组合"。你选出来的组合方式事后可以被归类为 Wrapper / Composer / Extractor / Facade / Coordinator——但你不是"先选了模式再设计"，而是"设计完了，发现它属于某个模式"。

初学者自然地倾向于"先选模式再做六步法"——这是倒置。正确的方向是：六步法走完，你得到的设计自然落入其中一个或几个模式。模式的价值在于沟通和教学（"这个 Hook 是个 Composer"比"这个 Hook 用了 useState + useEffect"传达了更多设计意图），但模式不是六步法的输入。

**Q3：步骤的"折叠"是正常的，取决于复杂度。**

Hook 的复杂度决定了六步法每一步的"重量"：
- **Wrapper**：Step 1-3 极轻（只有一个响应式值，分类就是 State，契约就是"语义化 API"），Step 4 几乎为零（依赖图只有一行），大部分精力在 Step 5-6。这是六步法的"轻量模式"——步骤一个没跳，但每一步的工作量极低。
- **Coordinator**：每一步都重。Step 1 可能识别出 5-10 个响应式值，Step 2 需要逐一分类并建立它们之间的关系，Step 4 需要画出完整的 Hook 依赖图。

步骤不会真的被"跳过"，只是复杂度低时自然地快速通过。如果某个步骤完全无法回答（比如 Step 1 "什么都不会变"），那你可能根本不需要 Hook——考虑用普通函数。
