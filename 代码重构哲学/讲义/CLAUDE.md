...
- 正文中不使用 emoji（课程进度标识如 🔴🟡🟢、检查清单如 ✅❌、重点标记如 ⚡ 等视觉编码场景允许）
...
- **标签**：在正文中使用 `#标签名`。嵌套用 `/` 如 `#设计/深模块`。大小写不敏感，搜索父标签匹配所有子标签。命名遵循 [标签命名规范](../../标签命名规范.md)
...
- **任务列表**：`- [ ]` / `- [x]` 追踪待办
- **转义**：`\` 取消特殊字符格式化效果，如 `\*\*不粗\*\*`
- **不使用**：脚注（插件已关闭）、HTML 标签（emoji 仅限视觉编码场景，见笔记风格）
